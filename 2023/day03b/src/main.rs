...
fn main() {
    ...
    for (schematic_row_index, schematic_row) in input.lines().enumerate() {
        for (schematic_col_index, schematic_symbol) in schematic_row.chars().enumerate() {
            ...
            above_values.dedup();
            above_values = above_values.iter().filter(|&&v| v != 0).cloned().collect::<Vec<u32>>();
            ...
            let mut same_row_values: Vec<u32> = vec![];
            for same_row_key in keys_in_same_row {
                let value = table.get(&same_row_key).map_or(0, |&v| v);
                same_row_values.push(value);
            }
            same_row_values = same_row_values.iter().filter(|&&v| v != 0).cloned().collect::<Vec<u32>>();
            ...
            below_values.dedup();
            below_values = below_values.iter().filter(|&&v| v != 0).cloned().collect::<Vec<u32>>();
            //result += below_values.into_iter().sum::<u32>();

            if above_values.iter().count() + same_row_values.iter().count() + below_values.iter().count() == 2 {
                above_values.extend(same_row_values);
                above_values.extend(below_values);
                result += above_values[0] * above_values[1]
            }
